...
while True:
    result = menu("Number convert menu", {
        1: 'Arabic number to Roman number',
        2: 'Roman number to Arabic number',
        0: 'Exit'
    })
    if result == 1:
        ar_to_rom = input("Enter arabic number less or equal to 3999: ")
        if int(ar_to_rom) > 3999:
            print()
            print(">>> Roman number greater than 3999 does not exist! <<<")
            print()
        else:
            print()
            print(f'>>> Your result is: {convert(int(ar_to_rom))} <<<')
            print()
        continue
    if result == 2:
        rom_to_ar = input("Enter roman number: ").capitalize()
        try:
            print()
            print(f'>>> Your result is: {convert(rom_to_ar)} <<<')
            print()
        except ValueError:
            print()
            print('>>> This number does not exist in Roman numeric system! <<<')
            print()
        continue
    if result == 0:
        break
